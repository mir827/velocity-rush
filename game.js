...
function step(dt){if(mode!=='playing')return;elapsed+=dt;noticeTimer-=dt;if(noticeTimer<=0)$('notice').textContent='';player.inv=Math.max(0,player.inv-dt);const dir=Number(keys.right)-Number(keys.left),max=keys.boost?440:220;player.vx+=dir*(player.grounded?420:300)*dt;if(!dir)player.vx*=Math.exp(-(player.grounded?2.4:.18)*dt);if(player.grounded)player.vx-=(ground(player.x+2)-ground(player.x-2))/4*120*dt;player.vx=Math.max(-max,Math.min(max,player.vx));
 ...
 if(!reduced){trail.push({x:player.x,y:player.y,life:.22});}trail.forEach(p=>p.life-=dt);trail=trail.filter(p=>p.life>0);particles.forEach(p=>{p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=500*dt;p.life-=dt;});particles=particles.filter(p=>p.life>0);cam+=(Math.max(0,Math.min(END-view+220,player.x-view*.36+Math.max(-45,Math.min(75,player.vx*.2))))-cam)*Math.min(1,dt*6);updateHUD();}
function formatTime(t){return `${Math.floor(t/60)}:${String(Math.floor(t%60)).padStart(2,'0')}`;}
function updateHUD(){$('lanterns').textContent=beacons.filter(b=>b.got).length+'/3';$('score').textContent=String(score).padStart(3,'0');$('time').textContent=formatTime(elapsed);$('speed').style.width=`${Math.abs(player.vx)/440*100}%`;}
